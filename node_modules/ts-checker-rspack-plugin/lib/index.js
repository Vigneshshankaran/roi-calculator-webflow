...
(()=>{
    ...
    function resolveTypeScriptGoPackageJsonPath(config) {
        if (!external_node_path_default().isAbsolute(config.typescriptPath) || 'package.json' !== external_node_path_default().basename(config.typescriptPath)) throw new Error(`The typescriptPath option must be an absolute path to "${TYPESCRIPT_GO_PACKAGE_JSON}" when tsgo is enabled.`);
    ...
    async function resolveTypeScriptGoBinPath(config) {
        ...
        const getExePathPath = external_node_path_default().resolve(external_node_path_default().dirname(tsgoPkgPath), './lib/getExePath.js');
        ...
        if ('function' != typeof getExePath) throw new Error(`Cannot resolve the typescript-go executable from "${TYPESCRIPT_GO_PACKAGE}".`);
        return getExePath();
    }
    ...
    async function runTypeScriptGo(config, logger, defaultSeverity = 'auto', signal) {
        ...
        return new Promise((resolve, reject)=>{
            let settled = false;
            let output = '';
            const childProcess = (0, external_node_child_process_namespaceObject.spawn)(binPath, args, {
                cwd: config.context,
                stdio: [
                    'inherit',
                    'pipe',
    ...
    function isDefaultTypeScriptGoPath(typescriptPath) {
        if (typescriptPath === TYPESCRIPT_GO_PACKAGE_JSON) return true;
        try {
    ...
    function createTypeScriptGoSupportError(config, error) {
        ...
        if (!isDefaultTypeScriptGoPath(config.typescriptPath)) message.push(`If you set \`typescript.typescriptPath\`, it must be an absolute path to \`${TYPESCRIPT_GO_PACKAGE_JSON}\`.`);
        if (error instanceof Error && error.message) message.push(`Failed to resolve the tsgo executable: ${error.message}`);
    ...
    function resolveDefaultTypeScriptPath(tsgo) {
        if (true === tsgo) try {
    ...
    function createTypeScriptWorkerConfig(compiler, options) {
        ...
        const typescriptPath = optionsAsObject.typescriptPath || resolveDefaultTypeScriptPath(optionsAsObject.tsgo);
        return {
